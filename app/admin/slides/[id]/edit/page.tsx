...
import { HeroSlide, HeroSlideFormData } from '@/app/types/heroSlide';
...
export default function EditSlidePage({ params }: EditSlidePageProps) {
  ...
  const fetchSlide = async () => {
    try {
      const response = await fetch(`/api/slides/${resolvedParams.id}`);
      if (!response.ok) {
        throw new Error('Failed to fetch slide');
      }
      
      const slide: HeroSlide = await response.json();
      setFormData({
        title: slide.title,
      ...
      <div className="md:grid md:grid-cols-3 md:gap-6">
        <div className="md:col-span-1">
          <div className="px-4 sm:px-0">
            <h3 className="text-lg font-medium leading-6 text-gray-900">Edit Hero Slide</h3>
